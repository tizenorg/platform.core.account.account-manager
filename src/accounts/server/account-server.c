...
int __get_information_for_cynara_check(GDBusMethodInvocation *invocation, char **client_smack, char **uid, char **session)
{
	...
	ret = cynara_creds_gdbus_get_user(gdbus_conn, sender, CLIENT_METHOD_SMACK, uid);
	...
	ret = cynara_creds_gdbus_get_client(gdbus_conn, sender, CLIENT_METHOD_SMACK, client_smack);
	if (ret != CYNARA_API_SUCCESS)
	{
		_ACCOUNT_FREE(sender);
...
int _check_privilege(GDBusMethodInvocation *invocation, const char* privilege)
{
	...
	if ( ret != ACCOUNT_ERROR_NONE )
	{
		...
		_ACCOUNT_FREE(client_smack);
		_ACCOUNT_FREE(uid);
		_ACCOUNT_FREE(session);
		return ACCOUNT_ERROR_PERMISSION_DENIED;
	}

	ret = __check_privilege_by_cynara(client_smack, session, uid, privilege);
	if ( ret != ACCOUNT_ERROR_NONE )
	{
